...
def get_predictions(adata, Mu_tensor, target_label, device, num_parts=50, ce_weight=0.1, epochs=100):
...
if __name__ == "__main__":
    ...
    # Targeting the highly granular 39-class label
    target_label = 'Cell_Type'

    # --- COLOR SYNCHRONIZATION ---
    all_global_cell_types = np.unique(df[target_label].astype(str))
    # Using HUSL to generate 39 highly distinct colors to avoid repeating tab20
    color_palette = sns.color_palette("husl", len(all_global_cell_types))
    global_color_dict = dict(zip(all_global_cell_types, [f"#{int(r*255):02x}{int(g*255):02x}{int(b*255):02x}" for r, g, b in color_palette]))

    regions_to_explore = ["05_06_23_reg001.tsv", "05_06_23_reg002.tsv", "05_06_23_reg003.tsv"]
    ...
    for target_region in regions_to_explore:
        ...
        # Run unsupervised discovery (Asking for slightly more latent capacity due to 39 true classes)
        print(" -> Discovering Latent Clusters...")
        mu_discovered = run_dir_vgae_and_extract_mu(adata, device, num_latent_clusters=35, epochs=150)
        
        # Run predictions (Locking to 100 parts and 0.1 CE based on your previous melanoma benchmark)
        print(" -> Running spGAT Spatial Deconvolution...")
        raw_clusters, mapped_labels = get_predictions(adata, mu_discovered, target_label, device, num_parts=100, ce_weight=0.1)
        ...
        cluster_breakdown = pd.crosstab(adata.obs['Raw_Latent_Cluster'], adata.obs[target_label])
        
        csv_name = f"Melanoma_{target_region.split('.')[0]}_Latent_to_Human_Mapping.csv"
        ...
        region_labels = np.unique(adata.obs[target_label])
        cm = confusion_matrix(adata.obs[target_label], adata.obs['spGAT_Mapped'], labels=region_labels)
        cm_df = pd.DataFrame(cm, index=region_labels, columns=region_labels)
        
        # Adjust heatmap size to comfortably fit 39 labels
        ...
        plt.tight_layout()
        cm_filename = f"Melanoma_{target_region.split('.')[0]}_Confusion_Matrix.png"
        ...
        adata.obs[target_label] = adata.obs[target_label].astype('category')
        adata.obs['spGAT_Mapped'] = adata.obs['spGAT_Mapped'].astype('category')
        
        adata.uns[f'{target_label}_colors'] = [global_color_dict[cat] for cat in adata.obs[target_label].cat.categories]
        adata.uns['spGAT_Mapped_colors'] = [global_color_dict[cat] for cat in adata.obs['spGAT_Mapped'].cat.categories]

        fig, axes = plt.subplots(1, 2, figsize=(20, 8))
        
        # Ground Truth
        sc.pl.spatial(adata, color=target_label, spot_size=15, ax=axes[0], show=False)
        axes[0].set_title(f"Ground Truth: 39 Classes ({target_region})")
        ...
        plt.tight_layout()
        spatial_filename = f"Melanoma_{target_region.split('.')[0]}_Synchronized_Spatial.png"
        plt.savefig(spatial_filename, dpi=300)
        plt.close()
        print(f"Saved Spatial Comparison to {spatial_filename}")
        
    print("\nFinished exploring all Melanoma regions.")