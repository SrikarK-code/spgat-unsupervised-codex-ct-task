...
# # --- (Keep Dir_Encoder_GCN, DirVGAE, Batched_STHD_SpGAT_Cosine, get_subgraphs here) ---
...
# # --- HELPER FUNCTION: MICROENVIRONMENT DISCOVERY ---
# def discover_environments(adata, label_col, A_env, num_communities=10):
...
#     print(f"\n--- Running Comparative Deep Analysis on {target_region} ---")
...
#     mapped_labels, ari_base, f1_base = get_predictions(adata, mu_discovered, "Cell Type", device, k_neighbors=6)
...
#     for label_col, title_prefix in [("Cell Type", "Ground Truth"), ("spGAT_Predicted", "Unsupervised spGAT")]:
...
#         g = sns.clustermap(mean_exp_z, cmap='coolwarm', center=0, figsize=(14, 10), metric='correlation', 
#                            cbar_pos=(0.02, 0.8, 0.05, 0.18))
#         g.fig.suptitle(f"Intestine ({target_region}): {title_prefix} Mean Expression", y=1.05)
#         plt.savefig(f"Intestine_{target_region}_{title_prefix.replace(' ', '_')}_Marker_Expression.png", dpi=300, bbox_inches='tight')
#         plt.close('all')

#     # ---------------------------------------------------------
#     # ANALYSIS 2: COMPARATIVE MICROENVIRONMENT DISCOVERY
#     # ---------------------------------------------------------
...
#     # Discover for Ground Truth
#     gt_comm, gt_enrich = discover_environments(adata, "Cell Type", A_env, num_communities)
#     adata.obs['GT_Microenvironment'] = gt_comm

#     # Discover for spGAT
#     pred_comm, pred_enrich = discover_environments(adata, "spGAT_Predicted", A_env, num_communities)
#     adata.obs['spGAT_Microenvironment'] = pred_comm

#     # Plot Comparative Enrichment Heatmaps
#     fig, axes = plt.subplots(1, 2, figsize=(24, 8))
#     sns.heatmap(gt_enrich, cmap='PRGn', center=0, ax=axes[0])
#     axes[0].set_title(f"Ground Truth Enrichment (Log2)")
#     axes[0].set_ylabel("Discovered Microenvironment")
    
#     sns.heatmap(pred_enrich, cmap='PRGn', center=0, ax=axes[1])
#     axes[1].set_title(f"Unsupervised spGAT Enrichment (Log2)")
#     axes[1].set_ylabel("") # Hide to save space
    
#     plt.tight_layout()
#     plt.savefig(f"Intestine_{target_region}_Comparative_Enrichment.png", dpi=300, bbox_inches='tight')
#     plt.close()
    
#     # Plot Comparative Spatial Communities
#     # Synchronize Colors
#     all_comms = np.unique(list(gt_comm) + list(pred_comm))
#     color_palette = sns.color_palette("tab10", len(all_comms))
#     comm_color_dict = dict(zip(all_comms, [f"#{int(r*255):02x}{int(g*255):02x}{int(b*255):02x}" for r, g, b in color_palette]))
    
#     adata.obs['GT_Microenvironment'] = adata.obs['GT_Microenvironment'].astype('category')
#     adata.obs['spGAT_Microenvironment'] = adata.obs['spGAT_Microenvironment'].astype('category')
#     adata.uns['GT_Microenvironment_colors'] = [comm_color_dict[cat] for cat in adata.obs['GT_Microenvironment'].cat.categories]
#     adata.uns['spGAT_Microenvironment_colors'] = [comm_color_dict[cat] for cat in adata.obs['spGAT_Microenvironment'].cat.categories]

#     fig, axes = plt.subplots(1, 2, figsize=(20, 8))
#     sc.pl.spatial(adata, color="GT_Microenvironment", spot_size=15, ax=axes[0], show=False)
#     axes[0].set_title(f"Ground Truth Microenvironments ({target_region})")
    
#     sc.pl.spatial(adata, color="spGAT_Microenvironment", spot_size=15, ax=axes[1], show=False)
#     axes[1].set_title(f"Unsupervised spGAT Microenvironments ({target_region})")
    
#     plt.tight_layout()
#     plt.savefig(f"Intestine_{target_region}_Comparative_Spatial_Microenvironments.png", dpi=300, bbox_inches='tight')
#     plt.close()

#     # ---------------------------------------------------------
#     # ANALYSIS 3: SPATIAL THRESHOLD ABLATION (Unchanged)
#     # ---------------------------------------------------------
...
#         for s in seeds:
#             set_seed(s)
#             _, ari, f1 = get_predictions(adata, mu_discovered, "Cell Type", device, k_neighbors=k)
...
#     fig.tight_layout()
#     plt.savefig(f"Intestine_{target_region}_Neighborhood_Ablation.png", dpi=300, bbox_inches='tight')
#     plt.close()
    
#     print("\nIntestine comparative analysis complete! Check the generated PNG files.")
...
def discover_and_map_environments(adata, label_col, A_env, gt_env_col, num_communities):
    ...
    # # 2. Map to Ground Truth Curated Neighborhoods via Bipartite matching
    # mapping = pd.crosstab(pd.Series(raw_labels), adata.obs[gt_env_col]).idxmax(axis=1).to_dict()
    # mapped_labels = pd.Series(raw_labels).map(mapping).values

    # 2. Map to Ground Truth Curated Neighborhoods via Bipartite matching
    # Add .values to both sides to ignore pandas indices and force 1-to-1 array alignment
...
if __name__ == "__main__":
    ...
    target_region = "B004_Descending"
    # Ensure the neighborhood column name matches your CSV exactly
    ...
    set_seed(43)
    mu_discovered = run_dir_vgae_and_extract_mu(adata, device)
    mapped_labels, ari_base, f1_base = get_predictions(adata, mu_discovered, "Cell Type", device, k_neighbors=6, num_parts=100)
    adata.obs['spGAT_Predicted'] = mapped_labels
    ...
    print("2. Discovering Comparative Microenvironments...")
    A_env = kneighbors_graph(adata.obsm['spatial'], n_neighbors=10, mode='connectivity', include_self=True)
    num_communities = len(adata.obs[gt_neighborhood_col].unique()) # dynamically match K to the number of GT communities
    ...
    gt_comm, gt_enrich = discover_and_map_environments(adata, "Cell Type", A_env, gt_neighborhood_col, num_communities)
    ...
    color_palette = sns.color_palette("tab20", len(all_comms))
    comm_color_dict = dict(zip(all_comms, [f"#{int(r*255):02x}{int(g*255):02x}{int(b*255):02x}" for r, g, b in color_palette]))
    ...
    plt.tight_layout()
    plt.savefig(f"Intestine_{target_region}_3Way_Spatial_Microenvironments.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for k in k_values:
        for s in seeds:
            set_seed(s)
            _, ari, f1 = get_predictions(adata, mu_discovered, "Cell Type", device, k_neighbors=k, num_parts=100)
            results.append({'K_Neighbors': k, 'Seed': s, 'ARI': ari, 'F1': f1})
            print(f"   Tested K={k}, Seed={s} -> F1: {f1:.4f}")
