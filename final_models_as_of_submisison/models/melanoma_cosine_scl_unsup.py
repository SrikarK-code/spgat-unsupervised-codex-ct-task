...
# --- 1. DIR-VGAE ARCHITECTURE (Unsupervised Discovery) ---
...
# --- 3. ROBUST COSINE-EM ARCHITECTURE ---
...
def run_dir_vgae_and_extract_mu(adata, device, num_latent_clusters=10, epochs=150):
    ...
    # Calculate average profile for each discovered Leiden cluster
    profiles = []
    for c in adata_tmp.obs['leiden'].unique():
        if sum(adata_tmp.obs['leiden'] == c) > 0:
            profiles.append(np.mean(X_raw[adata_tmp.obs['leiden'] == c], axis=0))
            
    print(f"     [Dir-VGAE] Discovered {len(profiles)} active latent states via Leiden.")
...
def train_and_eval_unsup(adata, Mu_tensor, target_label, device, num_parts=50, ce_weight=0.1, epochs=100):
    ...
    # Bipartite matching back to ground truth
...
if __name__ == "__main__":
    ...
    # Setting target label to Overall_Cell_Type (Tumor, Immune, Stromal, Epithelial)
    target_label = 'Overall_Cell_Type' 

    csv_file = "melanoma_fully_unsupervised.csv"
    with open(csv_file, "w") as f:
        f.write("Image,Num_Parts,CE_Weight,ARI,F1\n")
    ...
    # We will loop through the first 5 images just to get quick baseline results
    all_images = df['filename'].unique()[:5] 

    print(f"\n[1] Running FULLY UNSUPERVISED Evaluations for {len(all_images)} images...")
    for target in all_images:
        ...
        # Ask it to discover up to 10 latent classes
        ...
        with open(csv_file, "a") as f:
            f.write(f"{target},{best_params['num_parts']},{best_params['ce_weight']},{ari:.4f},{best_f1:.4f}\n")
