...
#     # Removed .T so STHD receives shape (Genes, Cell_Types) correctly
...
#     mapping = pd.crosstab(a_final.obs["STHD_pred_ct"], a_final.obs["Cell Type"]).idxmax(axis=1).to_dict()
#     a_final.obs["Mapped"] = a_final.obs["STHD_pred_ct"].map(mapping)
...
#     csv_file = "sthd_scale_ablations_fixed.csv"
#     with open(csv_file, "w") as f: 
#         f.write("Scale,Region,Total_ARI,Total_F1,Clean_ARI,Clean_F1,Mixed_ARI,Mixed_F1\n")

#     # 1. UNIVERSAL ABLATION (With Fix)
#     print("\n--- RUNNING UNIVERSAL ABLATION (FIXED) ---")
#     univ_dict_path = "universal_dict_sthd_fixed.tsv"
...
#     # 2. LOCAL ABLATION (With Fix)
#     print("\n--- RUNNING LOCAL ABLATION (FIXED) ---")
#     for reg in df['unique_region'].unique():
#         df_sub = df[df['unique_region'] == reg]
#         if len(df_sub) < 10: continue
#         print(f"\nProcessing LOCAL Region: {reg} ({len(df_sub)} cells)")
#         adata = ad.AnnData(X=df_sub[markers].values, obs=df_sub.drop(columns=markers))
#         adata.var_names = markers
#         adata.obs_names = [f"{reg}_{i}" for i in range(len(adata))]
#         adata.obsm['spatial'] = df_sub[['x', 'y']].values
...
#         with open(csv_file, "a") as f: 
#             f.write(f"LOCAL,{reg},{res[0]:.4f},{res[1]:.4f},{res[2]:.4f},{res[3]:.4f},{res[4]:.4f},{res[5]:.4f}\n")
#         print(f"  -> Total ARI: {res[0]:.4f} | Clean ARI: {res[2]:.4f}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(csv_file):
        ...
        for _, row in df_existing.iterrows():
            completed_runs.add((row['Scale'], row['Region']))
    else:
        with open(csv_file, "w") as f: 
            f.write("Scale,Region,Total_ARI,Total_F1,Clean_ARI,Clean_F1,Mixed_ARI,Mixed_F1\n")
    ...
    for reg in df['unique_region'].unique():
        if ("UNIVERSAL", reg) in completed_runs:
            continue
            
        df_sub = df[df['unique_region'] == reg]
        if len(df_sub) < 10: continue
        print(f"\nProcessing UNIVERSAL Region: {reg} ({len(df_sub)} cells)")
        adata = ad.AnnData(X=df_sub[markers].values, obs=df_sub.drop(columns=markers))
        adata.var_names = markers
        adata.obs_names = [f"{reg}_{i}" for i in range(len(adata))]
        adata.obsm['spatial'] = df_sub[['x', 'y']].values
        adata.obs = adata.obs.astype(str) # BUG FIX for HDF5 crash
    ...
    print("\n--- RUNNING LOCAL ABLATION (FIXED) ---")
    for reg in df['unique_region'].unique():
        if ("LOCAL", reg) in completed_runs:
            continue
