...
def run_spgat_config(adata, Mu_tensor, ei_space, config_name, beta, lr_W, lr_gat):
    print(f"\n{'-'*60}\nTesting Region: {config_name} | Beta: {beta} | LR_W: {lr_W} | LR_GAT: {lr_gat}")
...
if __name__ == "__main__":
    ...
    # Load Dictionary Once (It applies globally to all regions)
    ...
    print(f"Found {len(unique_regions)} unique regions to process.")

    # Your best config values
    b, lr_w, lr_gat = 0.01, 0.1, 0.01
    
    results = []
    csv_file = "spgat_all_regions_results.csv"
    with open(csv_file, "w") as f:
        f.write("Region,Total_ARI,Total_F1,Clean_ARI,Clean_F1\n")

    # Iterate over every unique region in the dataset
    for region in unique_regions:
        print(f"\n{'='*80}\nProcessing Region: {region}\n{'='*80}")
        
        df_sub = df_HuBMAP[df_HuBMAP['unique_region'] == region]
        
        # Skip regions that are fundamentally too small to graph
        if len(df_sub) < 10:
        ...
        # VERY IMPORTANT: Build spatial graph ONLY for this specific region slice
        print(f"Building Spatial Adjacency Matrix for {region} ({len(df_sub)} cells)...")
        ...
        # Run the model for this region
        ari_t, f1_t, ari_c, f1_c = run_spgat_config(adata.copy(), Mu_tensor, ei_space, region, b, lr_w, lr_gat)
        
        # Save incremental results
        results.append((region, ari_t, f1_t, ari_c, f1_c))
        with open(csv_file, "a") as f:
            f.write(f"{region},{ari_t:.4f},{f1_t:.4f},{ari_c:.4f},{f1_c:.4f}\n")

    print("\n" + "="*80)
    print("FINAL SpGAT REGION LEADERBOARD (Sorted by Total ARI):")
    for res in sorted(results, key=lambda x: x[1], reverse=True):
        print(f"{res[0]:<30}: [TOTAL] ARI={res[1]:.4f} | [CLEAN] ARI={res[3]:.4f}")
    print("="*80)