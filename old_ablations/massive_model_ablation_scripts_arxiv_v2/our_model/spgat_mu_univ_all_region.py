...
def extract_mu_dict(X_raw, device):
...
if __name__ == "__main__":
    ...
    csv_file = "spgat_scale_ablations_universal.csv"
    with open(csv_file, "w") as f: 
        f.write("Scale,Region,Total_ARI,Total_F1,Clean_ARI,Clean_F1,Mixed_ARI,Mixed_F1\n")

    print("\n--- RUNNING UNIVERSAL ABLATION (SpGAT) ---")
    univ_dict_path = "universal_dict_spgat.pt"
    
    if os.path.exists(univ_dict_path):
        print("Loading cached Universal Dictionary...")
        cache = torch.load(univ_dict_path)
        Mu_univ = cache['Mu_univ'].to(device)
    else:
        ...
        Mu_univ = extract_mu_dict(df_sample[markers].values, device)
        torch.save({'Mu_univ': Mu_univ.cpu()}, univ_dict_path)
