...
if __name__ == "__main__":
    ...
    detailed_csv = "stellar_per_class_metrics.csv"
    ...
    # MODIFICATION: Create the spatial predictions CSV file
    spatial_preds_csv = "stellar_spatial_predictions.csv"
    with open(spatial_preds_csv, "w") as f:
        f.write("Target_Region,x,y,Ground_Truth,STELLAR_Pred\n")
    ...
    for d in inter_donors:
        df_d = df[df['donor'] == d].sample(n=min(len(df[df['donor'] == d]), 20000), random_state=43) 
    ...
    for region in unique_regions:
        ...
        report_intra = classification_report(true_labels_str, pred_str_intra, output_dict=True, zero_division=0)
        save_per_class_metrics(detailed_csv, region, "INTRA_DONOR_STELLAR", report_intra)
        
        # MODIFICATION: Save the STELLAR spatial predictions to CSV
        df_preds = pd.DataFrame({
            'Target_Region': region,
            'x': df_sub['x'].values,
            'y': df_sub['y'].values,
            'Ground_Truth': true_labels_str,
            'STELLAR_Pred': pred_str_intra
        })
        df_preds.to_csv(spatial_preds_csv, mode='a', header=False, index=False)
        print(f"  => Appended spatial predictions to {spatial_preds_csv}")
        ...
        for d in inter_donors:
            ...
            report_macro = classification_report(true_labels_str, pred_str_macro, output_dict=True, zero_division=0)
            save_per_class_metrics(detailed_csv, region, f"INTER_DONOR_{d}_STELLAR", report_macro)

    print("\nFinished all STELLAR Per-Class evaluations.")