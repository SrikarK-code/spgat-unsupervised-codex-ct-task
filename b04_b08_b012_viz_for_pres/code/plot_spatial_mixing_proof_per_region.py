...
def calculate_mixing_and_plot_per_region(raw_data_csv, spgat_metrics, stellar_metrics, donor, out_dir):
    ...
    # Calculate mixing strictly per region
    ...
    full_plot_df = pd.merge(df_metrics, avg_mixing, on=['Target_Region', 'Cell_Type'])
    
    # Filter out extreme noise (cells with less than 10 instances in a region)
    full_plot_df = full_plot_df[full_plot_df['Support_Count'] >= 10]
    
    if len(full_plot_df) == 0: return
    
    # Determine winner for coloring
    full_plot_df['Winner'] = np.where(full_plot_df['Delta_F1'] > 0, 'spGAT Won', 'STELLAR Won')

    # 4. Plotting (NOW PER REGION)
    unique_regions = full_plot_df['Target_Region'].unique()
    colors = {'spGAT Won': '#ff7f0e', 'STELLAR Won': '#2ca02c'}
    
    os.makedirs(out_dir, exist_ok=True)
    
    for region in unique_regions:
        print(f" -> Generating Mixing Plot for region: {region}...")
        plot_df = full_plot_df[full_plot_df['Target_Region'] == region]
        
        # Skip if a region doesn't have enough cell types to make a meaningful scatter plot
        if len(plot_df) < 3: continue 
        
        plt.figure(figsize=(10, 7))
        sns.set_theme(style="whitegrid")
        
        # Draw regression line for the trend in this specific region
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f"Presentation_MixingProof_{safe_region_name}.png"), dpi=300)
        plt.close()

if __name__ == "__main__":
    # Point this to your master Dryad CSV
    ...
    for donor in ["B008", "B012"]:
        ...
        if os.path.exists(raw_csv) and os.path.exists(spgat_metrics):
            calculate_mixing_and_plot_per_region(raw_csv, spgat_metrics, stellar_metrics, donor, os.path.join(base_out_dir, donor))