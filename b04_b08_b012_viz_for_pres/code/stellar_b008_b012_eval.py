...
if __name__ == "__main__":
    ...
    for donor in donors_to_test:
        print(f"\n{'#'*60}\nEvaluating STELLAR on Donor: {donor}\n{'#'*60}")
        
        df_donor = df[df['donor'] == donor]
        unique_regions = df_donor['unique_region'].unique()
        
        # Dynamically pick the first region as the anchor (Intra-Donor Prior)
        anchor_reg = unique_regions[0]
        target_regions = [r for r in unique_regions if r != anchor_reg]
        
        print(f"[{donor}] Selected Anchor Region for STELLAR: {anchor_reg}")
        
        detailed_csv = f"{donor}_stellar_per_class_metrics.csv"
        spatial_preds_csv = f"{donor}_stellar_spatial_predictions.csv"
        ...
        X_intra, y_intra, edges_intra = extract_stellar_data(df[df['unique_region'] == anchor_reg], markers, le)

        for region in target_regions:
            df_sub = df[df['unique_region'] == region]
            ...
            print(f"  -> Testing Intra-Donor Prior ({anchor_reg})...")
